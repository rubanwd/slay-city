...
import type { RhymeMatchContent } from "./types";

export interface RhymeMatchTaskProps {
  content: RhymeMatchContent;
  onComplete: () => void;
  actionLabel?: string;
}
...
export default function RhymeMatchTask({
  ...
  const { prompt, targetWord, options, correctIndex } = content;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="text-center">
        <p className="text-small text-white/50">{prompt}</p>
        <p className="mt-1 text-h1 font-black text-cyan">{targetWord}</p>
      </div>
      ...
      {answered && (
        <p className={["text-center font-bold", isCorrect ? "text-lime-green" : "text-neon-pink"].join(" ")}>
          {isCorrect ? "Nice rhyme! 🎉" : "Not quite — the answer is highlighted."}
        </p>
      )}
