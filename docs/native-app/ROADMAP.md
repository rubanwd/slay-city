...
- `WP-0.1` Repository bootstrap: Expo app, TypeScript, NativeWind, CI skeleton, and
  `reference/web/` — the frozen snapshot of `rubanwd/slay-city`, excluded from
  build, lint, tests and `tsconfig.json`
...
sees that student's progress update. `reference/web/` is deleted — nothing is left
to port.
