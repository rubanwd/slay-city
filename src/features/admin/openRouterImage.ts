 ...
 * task illustrations) and teacher vocabulary flashcards. Gemini's image model
 * renders stylised illustrated scenes well and returns them inline as data
 * URLs, which is what the admin preview needs before anything is committed to
 * storage. Overridable per deploy with `OPENROUTER_IMAGE_MODEL` (mirrors
 * `OPENROUTER_TEXT_MODEL` in `openRouterChat.ts`); callers may also pass a
 * `model` argument for a one-off.
 */
const DEFAULT_OPENROUTER_MODEL =
  process.env.OPENROUTER_IMAGE_MODEL || "google/gemini-2.5-flash-image";

/**
 * Provider endpoint preference, as an OpenRouter provider routing slug.
 *
 * Image generation is by far the most expensive thing this app buys, and the
 * same model is sold at several service tiers. Google AI Studio's *flex* tier
 * serves the identical model at half the standard rate ($15 vs $30 per million
 * image-output tokens — roughly $0.019 vs $0.039 per 1024px picture), trading
 * guaranteed latency for a best-effort queue. Every image here is generated by
 * an admin or teacher who is already waiting on a preview, never by a student
 * mid-lesson, so slower-but-cheaper is the right trade.
 *
 * Flex buys that discount with higher latency *and lower availability*, so
 * fallbacks stay on: naming the tier-suffixed slug here opts in, and if flex
 * has no capacity OpenRouter falls through to the standard endpoints instead of
 * failing the admin's form. (OpenRouter's `service_tier: "flex"` parameter is
 * the stricter alternative — it never falls back, surfacing a capacity error
 * instead, which is the wrong trade for someone waiting on a preview.) A
 * fallback can never land on the *more* expensive priority tier, since that one
 * needs its own opt-in. Set `OPENROUTER_IMAGE_PROVIDER` to another slug (or to
 * an empty string) to change tiers without a code change.
 */
const IMAGE_PROVIDER_SLUG =
  process.env.OPENROUTER_IMAGE_PROVIDER ?? "google-ai-studio/flex";

const OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions";

/**
 * Image generation is slow; fail loudly rather than hanging the admin's form.
 * The flex tier is a best-effort queue, so this is deliberately generous.
 */
...
export function imageProviderRouting(
  slug: string = IMAGE_PROVIDER_SLUG
