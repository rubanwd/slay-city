...
import { completeHomeworkVocab } from "./actions";
...
export interface VocabularyFlowProps {
  topicId: string;
  topicTitle: string;
  words: HomeworkWord[];
...
export default function VocabularyFlow({
  ...
  const savePass = () => {
    setError(null);
    startSave(async () => {
      const result = await completeHomeworkVocab(topicId);
      if (!result.ok) {
    ...
    return (
      <AppContainer>
        <Section pt="lg" pb="sm">
          ...
          <h1 className="text-h2 font-black text-white">Learn the Words</h1>
          <p className="mt-1 text-small text-white/60">
            {totalCards} word{totalCards === 1 ? "" : "s"} to study, then a {totalTasks}-task test.
          </p>
        </Section>
        ...
        <Section py="sm" className="gap-4">
          {passed && (
            <div className="flex items-center gap-2 rounded-2xl border border-lime-green/30 bg-lime-green/10 px-4 py-3">
              ...
              <p className="text-small font-bold text-lime-green">You&apos;ve passed these words.</p>
            </div>
          )}
        ...
        <Section pt="lg" pb="sm" className="shrink-0 gap-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="truncate text-[11px] font-bold uppercase tracking-[0.15em] text-white/50">
              Word {cardIndex + 1} of {totalCards}
            </span>
  ...
  return (
    <AppContainer>
      <Section pt="lg" className="items-center gap-4 text-center">
        ...
        <h1 className="text-h2 font-black text-white">Words passed!</h1>
