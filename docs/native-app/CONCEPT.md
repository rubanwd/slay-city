...
│    migrations/           │     │  reference/web/ ← frozen │
│    functions/            │     │                          │
└───────────┬──────────────┘     └───────────┬──────────────┘
            │                                │
            │        ┌───────────────┐       │
...
├── reference/
│   └── web/                  # frozen snapshot of the Next.js app
├── scripts/
...
### `reference/web/` — the snapshot

A frozen copy of `rubanwd/slay-city` at the commit the port started from,
excluded from the build, from `tsconfig.json`, from lint and from tests.
...
`reference/web/src/features/mission/QuizTask.tsx` and
`src/features/mission/QuizTask.tsx` is exactly the question a reviewer needs to
answer.

It is deleted at the end of M5, once nothing is left to port. It is never imported
from, never edited, and never updated — a snapshot that drifts silently is worse
than no snapshot.
