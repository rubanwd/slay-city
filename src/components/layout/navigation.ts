...
export interface NavItemOptions {
  /** Student only: shows the Homework tab when they belong to a teacher group. */
  showHomework?: boolean;
}

const STUDENT_ITEMS: NavItemModel[] = [
  { label: "Map", href: "/map", icon: "map" },
  { label: "Wardrobe", href: "/wardrobe", icon: "wardrobe" },
];

const STUDENT_HOMEWORK_ITEM: NavItemModel = {
  label: "Homework",
  href: "/homework",
  icon: "homework",
};

const STUDENT_PROFILE_ITEM: NavItemModel = { label: "Profile", href: "/profile", icon: "profile" };
...
export function navItemsForRole(
  ...
  return showHomework
    ? [...STUDENT_ITEMS, STUDENT_HOMEWORK_ITEM, STUDENT_PROFILE_ITEM]
    : [...STUDENT_ITEMS, STUDENT_PROFILE_ITEM];
}

/**
 * Index of the tab the current path belongs to, or -1 when none does (e.g. a
 * student inside a mission). A tab owns its own path and everything below it, and
